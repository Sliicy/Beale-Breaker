...
namespace Beale_Breaker {
    public partial class FrmMain : Form {
        ...
        private void TxtInput_TextChanged(object sender, EventArgs e) {
            // Sanitizes the input, leaving only alphabetical characters:
            ...
            // Holds a list of words from the input:
            List<string> words = new List<string>(str.Split(' '));

            if (words.Count >= key1.Max()) {
                txtKey1.Text = TransposeLetters(words, key1, cmbSortMethod.Text);
            }
            if (words.Count >= key2.Max()) {
                txtKey2.Text = TransposeLetters(words, key2, cmbSortMethod.Text);
            }
            if (words.Count >= key3.Max()) {
                txtKey3.Text = TransposeLetters(words, key3, cmbSortMethod.Text);
            }
        }
