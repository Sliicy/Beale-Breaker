...
namespace Beale_Breaker {
    public partial class FrmMain : Form {
        ...
        private string TransposeLetters (List<string> words, List<int> key) {
            string output = "";
            foreach (int number in key) {

                if (cmbSortMethod.Text == "First Letter") {

                    output += words[number].Substring(0, 1);

                } else if (cmbSortMethod.Text == "Last Letter") {

                    output += words[number].Substring(words[number].Length - 1, 1);

                } else if (cmbSortMethod.Text == "Whole Word") {
        ...
        private void TxtInput_TextChanged(object sender, EventArgs e) {
            ...
            if (words.Count >= key1.Max()) {
                txtKey1.Text = TransposeLetters(words, key1);
            }
            if (words.Count >= key2.Max()) {
                txtKey2.Text = TransposeLetters(words, key2);
            }
            if (words.Count >= key3.Max()) {
                txtKey3.Text = TransposeLetters(words, key3);
            }
        }
